...
class VisualFeatEncoder(nn.Module):
    def __init__(self, config, clip_name):
        super().__init__()
        ...
        self.visn_layer_norm = BertLayerNorm(config.d_model, eps=1e-12)

        # Box position encoding
        self.box_fc = nn.Linear(pos_dim, config.d_model)
        self.box_layer_norm = BertLayerNorm(config.d_model, eps=1e-12)

        self.dropout = nn.Dropout(config.dropout)

    def forward(self, visn_input):
        if isinstance(visn_input, tuple):  # False
...
class BartEncoder(nn.Module):
    ...
    def __init__(self, config: BartConfig, embed_tokens, clip_name):
        ...
        self.visn_fc = VisualFeatEncoder(config, clip_name)

        self.embed_positions_mm = LearnedPositionalEmbedding_mm(
            config.max_position_embeddings, embed_dim, self.padding_idx, config.extra_pos_embeddings,
        )
    ...
    def forward(self,
        ...
        """
        visn_feats = self.visn_fc(img_feature)  # torch.Size([1, 246, 1024])
        visn_feats = self.visual_pos(visn_feats, skip_last_layer=False, curr_img_len=None)  # torch.Size([1, 246, 2048])--# torch.Size([1, 246, 1024])

        bs, img_len = visn_feats.shape[:2]

        vis_attention_mask = torch.ones((bs, img_len)).to(int(cuda[-1]))

        mm_pad_mask = torch.cat((vis_attention_mask, para_attention_mask), dim=1)
        ...
        token_type_id_mm = torch.cat([torch.zeros_like(para_input_id), torch.ones((bs, img_len)).to(int(cuda[-1]))], dim=1).long()

        token_type_rep = self.token_type_embeddings(token_type_id_mm)

        # x = mm_embed + embed_pos
        x = torch.cat([x_text, visn_feats], dim=1)
        ...
        bs, vis_len = visn_feats.shape[:2]
        _, text_len = inputs_embeds.shape[:2]

        # x_t = torch.split(x, [vis_len, text_len], dim=1)[1]
        x_t = torch.split(x, [text_len, vis_len], dim=1)[0]
...
class BartEncoder_up2(nn.Module):
    ...
    def forward(self,
        ...
        if pair_attention_mask is not None:
            pair_attention_mask = invert_mask(pair_attention_mask)
        x = top_rep
...
class BartForOrdering_student(PretrainedBartModel):
    def __init__(self, config: BartConfig, **kwargs):
        ...
        #### clip for imgs #### # RN50 or ViT-B/32
        import clip
        img_len = 5
        # model_name = 'RN50'
        # # model_name = "ViT-B/32"
        reset_pos_embedding = False
        clip_model, _ = clip.load(model_name, jit=False, img_len=img_len, img_only=False)
        if model_name == "ViT-B/32" and reset_pos_embedding:
            pos_embed = nn.Parameter(torch.zeros(num_patches + 1, 768).float())
            pos_embed.weight = resize_pos_embed(clip_model.visual.positional_embedding.unsqueeze(0),
                                                pos_embed.unsqueeze(0))
            clip_model.visual.positional_embedding = pos_embed
        self.visual_model = clip_model
    ...
    def forward(self, input_ids, attention_mask=None, token_type_ids=None,
                ...
                para_input_id=None, para_attention_mask=None, max_sentence_length=None, imgs=None, sent_len_for_mask=None,
                mm_mask=None,
        ...
        cls_score_matrix_nn, cls_score_matrix_nn_his1, cls_score_matrix_nn_his2, \
            ...
            = self.encode(input_ids, attention_mask, token_type_ids, pairs_list, passage_length, pairs_num,
                          ...
                          para_input_id, para_attention_mask, max_sentence_length, imgs, sent_len_for_mask, mm_mask,
                          cuda, head_mask)
        ...
        # print('key.shape', key.shape)  # torch.Size([4, 5, 5, 1024])
        ...
        batch, max_pair_num = pairwise_labels.size()  # (4, 20)
        ...
        pair_masks = torch.FloatTensor(pair_masks).cuda(int(cuda[-1]))  # [batch_size, max_pair_num]
        pair_masks_new = pair_masks.view(-1)

        loss = new_original_loss

        return loss, match_layer_hidden_s, cls_pooled_output, pair_masks_new, pair_senvec_top_s, \
               document_matrix, cls_score, match_layer_attention_s
    ...
    def encode(self, input_ids, attention_mask=None, token_type_ids=None,
        ...
        img_fea = self.visual_model.visual(imgs.type(self.visual_model.dtype),
        ...
        outputs = self.model(pair_input_ids=input_ids,
                            ...
                            img_feature=img_fea,
                            sent_len_for_mask=sent_len_for_mask,
                            mm_mask=mm_mask,
                            cuda=cuda,
                            use_cache=False,
                            )
        ...
        cls_pooled_output = outputs_up2[1]  # torch.Size([80, 1024])
    ...
    def step(self, dec_inputs, enc_output, original_keys, mask, rela_vec, rela_mask,
        ...
        # rela_vec_diret = rela_vec
        #
        # fut_score_forw = self.fut_tran_forw(rela_vec_diret)
        # fut_score_back = self.fut_tran_back(rela_vec_diret.transpose(1, 2))
        # # print('fut_score_forw.shape', fut_score_forw.shape)  # torch.Size([4, 5, 5, 1])
        # fut_score_forw.masked_fill_(rela_mask.unsqueeze(-1) == 0, -1e9)
        # fut_score_back.masked_fill_(rela_mask.unsqueeze(-1) == 0, -1e9)
        # forw_probs = nn.Softmax(dim=-2)(fut_score_forw)
        # back_probs = nn.Softmax(dim=-2)(fut_score_back)
        # # print(forw_probs.shape)  # torch.Size([4, 5, 5, 1])
        # forw_pw = rela_vec_diret * (forw_probs.masked_fill(rela_mask.unsqueeze(-1) == 0, 0))
        # back_pw = rela_vec_diret * (back_probs.masked_fill(rela_mask.unsqueeze(-1) == 0, 0))
        # # print('forw_pw.shape', forw_pw.shape)  # torch.Size([4, 5, 5, 1026])
        # forw_futu = forw_pw.sum(2)
        # back_futu = back_pw.sum(2)
        # print('forw_pw.shape', forw_pw.shape)  # torch.Size([4, 5, 1026])
...
def beam_search_pointer(args, model, input_ids, attention_mask=None, token_type_ids=None,
                ...
                sent_len_for_mask=None, mm_mask=None,
                cuda=None, head_mask=None):
